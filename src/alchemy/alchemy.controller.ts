...
export class AlchemyController {
    ...
    async getTokenHistory(@Param('token') token: string, @Body() body: { beginDate: string; endDate: string; interval: Number}): Promise<any> {
        const beginDate = new Date(body.beginDate);
        const endDate = new Date(body.endDate);
        return this.alchemyService.getTokenHistoricPrice(token, beginDate, endDate, body.interval);
    }
}
